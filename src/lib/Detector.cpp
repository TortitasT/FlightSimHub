...
std::vector<std::filesystem::path> ParseSteamLibraryFolders(
  ...
  static const std::regex pattern {R"("path"\s+"([^"]+)")"};
...
std::vector<std::filesystem::path> Detector::SteamCommonDirs() const {
  ...
  for (const auto& library: ParseSteamLibraryFolders(*vdfText)) {
    dirs.push_back(library / "steamapps" / "common");
  }
  return dirs;
}
...
InstallState Detector::Resolve(
  ...
  const auto managed = mManagedAppsDir / app.id / app.exeName;
